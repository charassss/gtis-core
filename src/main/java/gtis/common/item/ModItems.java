package gtis.common.item;

import net.minecraft.item.Item;

import java.util.ArrayList;
import java.util.List;

/**
 * @author Chara_SS
 */

@SuppressWarnings("unused")
public class ModItems {
    public static final List<Item> ITEMS = new ArrayList<>();
    public static final Item ORI_ROCK_CUBE = new ItemBase("ori_rock_cube");
    public static final Item MANA_IRON_INGOT = new ItemBase("mana_iron_ingot");
    public static final Item UNCHARGED_TERRASTEEL_INGOT = new ItemBase("uncharged_terrasteel_ingot");
    public static final Item ZHU_FU_RONG_GUANG_CD = new ZhuFuRongGuang("zhu_fu_rong_guang_cd");


}
